...
def create_data(w, b, nums_example):
    X = torch.normal(0, 1, (nums_example, len(w)))#返回正态分布的张量，1000*2个
...
def read_data(batch_size, features, lables):
    ...
    for i in range(0, nums_example, batch_size):  # range(start, stop, step)#10个10个
        index_tensor = torch.tensor(indices[i: min(i + batch_size, nums_example)])
        #print("index_tensor_shape:",index_tensor.shape)#index_tensor是10*1
...
# 训练模型
lr = 0.03#测试后发现loss=0.000048已经最小了
num_epochs = 10#只会计算3次loss里的最低
